...
func SaveImageFromVideo(coverName *string, isDebug bool) error {
	v2i := NewVideo2Image()
	if isDebug {
		v2i.Debug()
	}

	v2i.InputPath = filepath.Join(config.Global.StaticSourcePath, *coverName+defaultVideoSuffix)
	*coverName += defaultImageSuffix
	v2i.OutputPath = filepath.Join(config.Global.StaticSourcePath, *coverName)
	v2i.FrameCount = 1
